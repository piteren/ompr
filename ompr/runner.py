...
from ompr.worker import RunningWorker, RWW
...
class OMPRunner:
    ...
    class InternalProcessor(ExProcess):
        ...
        def _build_and_start_RWW(self, rww_name:str):
            ...
            logger.debug(f'> {self.name} built and started {rww_name} ..')

        def build_and_start_allRWW(self):
            logger.debug(f'> {self.name} is going to build and start {len(self.rwwD)} RunningWorkers ..')
            n_started = 0
            for id in self.rwwD:
        ...
        def _stop_and_join_RWW(self, name:str):
            ...
            logger.debug(f'> {self.name} stopped and joined {name} ..')

        def _stop_and_join_allRWW(self):
            logger.debug(f'> {self.name} is going to stop and join {len(self.rwwD)} RunningWorkers ..')
        ...
        def _kill_RWW(self, name:str):
            ...
            self.rwwD[name]['rww'] = None
            logger.debug(f'> {self.name} killed and joined {name} ..')

        def _kill_allRWW(self):
            logger.debug(f'> {self.name} is going to kill and join {len(self.rwwD)} RunningWorkers ..')
            for name in self.rwwD:
                if self.rwwD[name]['rww'] is not None:
                    self._kill_RWW(name)
        ...
        def _hold_till_allRWW_ready(self):
            ...
            for name in self.rwwD:
                if self.rwwD[name]['rww'] is None:
                    logger.warning('some RWW are not started, cannot hold!')
                    return
            for name in self.rwwD:
        ...
        def exprocess_method(self):
            ...
            while not break_ompr:
                ...
                logger.debug(f'> {self.name} got \'{msg_ique.type}\' message from ique')
                ...
                if msg_ique.type in ['RWW_result','RWW_exception']:
                    ...
                    logger.debug(f'> {self.name} got message from {rww_name} for task {task_ix}')
                    ...
                    if type(result) is OMPRException:
                        ...
                        if self.log_rww_exception:
                            logger.warning(f'> {self.name} got exception message from {rww_name} for task #{task_ix}: {result}')

                        logger.debug(f'> restarting RWW: {rww_name} because crashed task:{task_ix} ..')
                ...
                while resources and tasks_que:
                    ...
                    logger.debug(f'> free resources: {len(resources)}, tasks_que len: {len(tasks_que)}, ique.size: {self.ique.size}')
                    ...
                    if self.rww_lifetime and self.rwwD[rww_name]['n_tasks'] >= self.rww_lifetime:
                        logger.debug(f'> restarting RWW: {rww_name} because of lifetime condition ..')
                        self._kill_RWW(rww_name)
                        self._build_and_start_RWW(rww_name)
                ...
                if self.report_interval is not None and c_time-iv_time > self.report_interval:
                    ...
                    logger.debug(f'RWW n_tasks:')
                    for rk in sorted(self.rwwD.keys()):
                        logger.debug(f'{rk:2}: {self.rwwD[rk]["n_tasks"]}')

        def after_exception_handle_run(self):
            self._kill_allRWW()
            logger.debug(f'> {self.name} killed all RWW after exception occurred')
    ...
    def __init__(
        ...
        :param loglevel_subproc:
            logging level for pypaq.mpython.mptools logger """

        logging.getLogger('ompr.worker').setLevel(loglevel_worker)
        logging.getLogger('pypaq.mpython.mptools').setLevel(loglevel_subproc)

        if logger.level < 20:
            log_rww_exception = True

        logger.info(f'*** OMPRunner *** inits, pid: {os.getpid()}, rww_class: {rww_class.__name__}')
    ...
    def exit(self):
        if not self._exited:
            ...
            if self._n_results_returned != self._n_tasks_received:
                kill = True
                logger.warning(f'{self.__class__.__name__} exits while not all results were returned to user!')
            self._internal_processor.exit(kill=kill)
            logger.info(f'> internal processor stopped, {self.__class__.__name__} exits.')
            self._exited = True